...
# SQL Practice Server - Load Testing Script (runs inside VM)
SERVER_URL="${1:-http://localhost:9000}"
NUM_USERS="${2:-1000}"

echo "========================================================="
echo "     SQL Practice Server - Load Testing (VM Internal)    "
echo "========================================================="
echo "Configuration:"
...
test_user() {
    ...
    if [ -z "$SESSION_TOKEN" ]; then
        echo "$i|FAIL|0|login_failed" >> "$RESULTS_FILE"
        return
    fi

    # Test Question ID 1: Duplicate Emails
    # Wrong answer first
    curl -s -X POST "$SERVER_URL/api/execute" \
        -H "Content-Type: application/json" \
        -d "{\"session_token\":\"$SESSION_TOKEN\",\"question_id\":\"1\",\"user_sql\":\"SELECT * FROM table;\"}" \
    ...
    local SUBMIT_RESPONSE=$(curl -s -X POST "$SERVER_URL/api/execute" \
        -H "Content-Type: application/json" \
        -d "{\"session_token\":\"$SESSION_TOKEN\",\"question_id\":\"1\",\"user_sql\":\"SELECT Email FROM Person GROUP BY Email HAVING COUNT(*) > 1;\"}" \
        --max-time 30 2>/dev/null)
...
# Run all users in parallel using xargs
echo "Running load test with $NUM_USERS concurrent users..."
seq 1 $NUM_USERS | xargs -P 200 -I {} bash -c 'test_user "$@"' _ {}

# Wait for all to complete and give time for final writes
sleep 3
echo ""
...
if [ $SUCCESSFUL -gt 0 ]; then
    ...
    ACCURACY=$(echo "scale=2; $CORRECT * 100 / $SUCCESSFUL" | bc 2>/dev/null || echo "0")
else
    AVG_RESPONSE_TIME=0
    ACCURACY="0.00"
fi
